...
export default function AnswerText(props) {
  ...
  const [orignialArr, setOriginalArr] = useState([]);
  ...
  useEffect(() => {
    ...
    if (
      mainArr.length > 0 &&
      !doneFlag &&
      curArr != mainArr[mainArr.length - 1].question_text
    ) {
      setCurArr(
  ...
  const getData = (code) => {
    if (test) {
      ...
      postTranscript(
        ...
        .then((res) => {
          if (res && res.status) {
            if (getCookie("session_token")) setApiStatus(res.status);
            else {
            ...
            if (
              payloads[0].transcript[payloads[0].transcript.length - 1]
                .answer_text &&
              tempPos + 1 == payloads[0].transcript.length
            )
              setDoneFlag(true);
            setPos(tempPos);
  ...
  const updateData = (obj) => {
    if (test) {
        ...
        .then((res) => {
          {
            if (res && res.status) {
              ...
              const { payloads } = res;
              // tempMainArr[tempMainArr.length - 1] = payloads[0].transcript[0];
              ///setMainArr([...mainArr, payloads[0].transcript[0]]);
              if (
                payloads[0].transcript.length == 2 &&
                payloads[0].transcript[1].question_text != ""
              ) {
                setMainArr([...tempMainArr, payloads[0].transcript[1]]);
              } else {
                const tempOrder =
                  Math.floor(payloads[0].transcript[0].order) + 1;
                if (tempOrder < orignialArr.length) {
                  tempMainArr.push(orignialArr[tempOrder]);
      ...
      updateTranscript(
        id,
        // { ...obj, transcript_id: transcript.id },
        {
          is_complete: true,
        ...
        .then((res) => {
          if (res && res.status) {
            ...
            // tempMainArr[tempMainArr.length - 1] = payloads[0].transcript[0];
            ///setMainArr([...mainArr, payloads[0].transcript[0]]);
            // const index = payloads[0].transcript.findIndex(
            //   (item) => item.order === currentOrder
            // );
            const index = payloads[0].transcript.length - 1;
            if (orignialArr.length - 1 > payloads[0].transcript[index].order) {
              setCurrentOrder(index + 1);
              setMainArr([
                ...tempMainArr,
                orignialArr[payloads[0].transcript[index].order + 1],
              ]);
            } else {
              setTyping(true);
              setDoneFlag(true);
            ...
            // if (
            //   payloads[0].transcript.length == 2 &&
            //   payloads[0].transcript[1].answer_text != ""
            // ) {
            //   setMainArr([...tempMainArr, payloads[0].transcript[1]]);
            // } else {
            //   const tempOrder = Math.floor(payloads[0].transcript[0].order) + 1;
  ...
  const sendFunc = () => {
    if (text != "" && !doneFlag) {
      ...
      if (!test) {
        ...
        if (tempObj.context || tempObj.context == "") delete tempObj.context;
        if (tempObj.answer_id == "") delete tempObj.answer_id;
